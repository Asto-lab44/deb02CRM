...
const ProjectsGantt = () => {
  ...
  const [monthsRange, setMonthsRange] = React.useState(3); // 3 mois affichés
  ...
  const endDate = React.useMemo(() => {
    const d = new Date(startDate); d.setMonth(d.getMonth() + monthsRange); d.setDate(0); d.setHours(23, 59, 59, 999); return d;
  }, [startDate, monthsRange]);
  const totalDays = Math.ceil((endDate - startDate) / (1000 * 3600 * 24));
  const dayWidth = 22; // px par jour
  ...
  const visibleProjects = React.useMemo(() => {
    return projects.filter((p) => {
      ...
      const due = p.delivery_due ? new Date(p.delivery_due) : new Date();
  ...
  days.forEach((d, i) => {
    ...
    else months.push({ key, label: d.toLocaleDateString("fr-FR", { month: "long", year: "numeric" }), count: 1 });
  });

  const today = new Date(); today.setHours(0, 0, 0, 0);
  const todayOffset = Math.floor((today - startDate) / (1000 * 3600 * 24));

  // Position et largeur d'une barre pour un projet
  const barFor = (p) => {
    const created = new Date(p.created_at);
    const due = p.delivery_due ? new Date(p.delivery_due) : today;
    const start = Math.max(0, Math.floor((created - startDate) / (1000 * 3600 * 24)));
    const end = Math.min(totalDays, Math.floor((due - startDate) / (1000 * 3600 * 24)) + 1);
    return { left: start * dayWidth, width: Math.max(dayWidth, (end - start) * dayWidth) };
  };

  const prevMonth = () => { const d = new Date(startDate); d.setMonth(d.getMonth() - 1); setStartDate(d); };
  const nextMonth = () => { const d = new Date(startDate); d.setMonth(d.getMonth() + 1); setStartDate(d); };
  const goToday = () => { const d = new Date(); d.setDate(1); d.setMonth(d.getMonth() - 1); d.setHours(0, 0, 0, 0); setStartDate(d); };
    ...
    <div style={S.frame}>
      ...
      <aside style={S.sidebar}>
        <a href="/" style={{ ...S.brand, textDecoration: "none", color: "inherit" }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "#64748b" }}>Gantt projets</div>
          </div>
        </a>
        ...
        <div style={S.legend}>
          <div style={S.legendLabel}>Stages</div>
          {STAGES.map((s) => (
      ...
      <main style={S.main}>
        <header style={S.topbar}>
          <div>
            <h1 style={S.h1}>Vue Gantt — Projets</h1>
            <p style={S.h1sub}>{visibleProjects.length} projet{visibleProjects.length > 1 ? "s" : ""} sur la période · zoom 1 jour = {dayWidth}px</p>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <button onClick={prevMonth} style={S.navBtn}>‹</button>
            <button onClick={goToday} style={S.btnPrimary}>Aujourd'hui</button>
            <button onClick={nextMonth} style={S.navBtn}>›</button>
            <select value={monthsRange} onChange={(e) => setMonthsRange(parseInt(e.target.value, 10))} style={S.select}>
        ...
        {loading ? (
          ...
          <div style={{ padding: 40, textAlign: "center", color: "#94a3b8" }}>Aucun projet sur cette période.</div>
        ) : (
          <div style={S.ganttWrap}>
            {/* Colonne projets (sticky left) */}
            <div style={S.leftCol}>
              <div style={S.leftHeader}>Projet</div>
            ...
            <div style={S.ganttScroll}>
              <div style={{ width: totalDays * dayWidth }}>
                {/* Header mois */}
                <div style={S.monthsHeader}>
                  {months.map((m) => (
                    <div key={m.key} style={{ width: m.count * dayWidth, padding: "8px 6px", textAlign: "center", fontSize: 11.5, fontWeight: 700, color: "#0f172a", borderRight: "1px solid #e2e8f0", background: "#fafbfc", textTransform: "capitalize" }}>
                      {m.label}
                    </div>
                  ))}
                </div>
                ...
                <div style={S.daysHeader}>
                  {days.map((d, i) => {
                    ...
                    return (
                      <div key={i} style={{ width: dayWidth, padding: "6px 0", textAlign: "center", fontSize: 10, fontWeight: isToday ? 800 : 500, color: isToday ? "#3730a3" : isWeekend ? "#cbd5e1" : "#64748b", borderRight: "1px solid #f1f5f9", background: isWeekend ? "#fafbfc" : "#fff" }}>
                        {d.getDate()}
                      </div>
                    );
                  })}
                ...
                <div style={{ position: "relative" }}>
                  ...
                  {todayOffset >= 0 && todayOffset < totalDays && (
                    <div style={{ position: "absolute", left: todayOffset * dayWidth + dayWidth / 2, top: 0, bottom: 0, width: 2, background: "#dc2626", zIndex: 5, pointerEvents: "none" }} />
                  )}
                  {visibleProjects.map((p) => {
                    const bar = barFor(p);
                    const stage = STAGES.find((s) => s.k === p.stage) || STAGES[0];
                    const overdue = p.delivery_due && new Date(p.delivery_due) < today && p.stage !== "clos";
                    return (
                      <div key={p.id} style={{ ...S.ganttRow, position: "relative" }}>
                        {/* Background grille */}
                        {days.map((d, i) => (
                          <div key={i} style={{ width: dayWidth, borderRight: "1px solid #f8fafc", flexShrink: 0, background: (d.getDay() === 0 || d.getDay() === 6) ? "#fafbfc" : "transparent" }} />
                        ))}
                        {/* Barre projet */}
                        <a href={"/projet?id=" + encodeURIComponent(p.id)}
...
const S = {
  ...
  sidebar: { width: 220, background: "#fff", borderRight: "1px solid #eef1f5", display: "flex", flexDirection: "column", padding: "16px 12px", gap: 8, flexShrink: 0 },
  ...
  legend: { padding: "12px 0", marginTop: 12 },
  ...
  select: { padding: "7px 12px", border: "1px solid #e2e8f0", borderRadius: 7, fontSize: 12.5, color: "#0f172a", background: "#fff", cursor: "pointer", fontFamily: "inherit" },

  ganttWrap: { display: "flex", flex: 1, overflow: "hidden", background: "#fff" },
  leftCol: { width: 260, flexShrink: 0, borderRight: "2px solid #e2e8f0", background: "#fafbfc", overflowY: "auto" },
  leftHeader: { padding: "12px 14px", background: "#0f172a", color: "#fff", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5, height: 60, display: "flex", alignItems: "center", borderBottom: "1px solid #1e293b" },
  leftRow: { display: "block", padding: "8px 14px", borderBottom: "1px solid #eef1f5", height: 34, textDecoration: "none", color: "inherit" },
  ganttScroll: { flex: 1, overflowX: "auto", overflowY: "auto" },
  monthsHeader: { display: "flex", borderBottom: "1px solid #e2e8f0" },
  daysHeader: { display: "flex", borderBottom: "2px solid #0f172a" },
  ganttRow: { display: "flex", borderBottom: "1px solid #eef1f5", height: 34 },
};
