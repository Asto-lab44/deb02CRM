...
const ClientPage = () => {
  ...
  const [expandedStages, setExpandedStages] = React.useState(() => {
    try { return JSON.parse(localStorage.getItem("hubAstorya.pipeExpanded.v1") || "{}"); }
    catch (e) { return {}; }
  });
  const toggleStageExpanded = (k) => setExpandedStages((m) => {
    const next = { ...m, [k]: !m[k] };
    try { localStorage.setItem("hubAstorya.pipeExpanded.v1", JSON.stringify(next)); } catch (e) {}
    return next;
  });
  ...
  const saveEdit = async () => {
    ...
    const patch = {
      ...
      etablissements_secondaires: Array.isArray(editDraft.etablissements_secondaires)
        ? editDraft.etablissements_secondaires.filter((e) => e.adresse || e.ville || e.nom)
        : [],
          ...
          <section style={cliStyles.hero}>
            ...
            <div style={cliStyles.actionBar}>
              ...
              <button
                onClick={async () => {
                  const cid = urlId || display.id;
                  if (!cid) { if (window.HubToast) window.HubToast.warn("Identifiant client introuvable"); return; }
                  const ok = window.HubModal
                    ? await window.HubModal.confirm({
                        ...
                        okLabel: "Générer le ZIP", okStyle: "primary",
                      })
                  ...
                  try {
                    const res = await window.api.dataExport.downloadZip(cid, display.name);
                ...
                style={{ ...cliStyles.ghostBtn, background: "#fef3c7", borderColor: "#fcd34d", color: "#92400e", fontWeight: 600 }}
                title="Exporter toutes les données du client en ZIP (CSV + JSON) — réversibilité RGPD"
              >
                    ...
                    <div key={s.k}
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          ...
                          {opps.length > 2 && (
                            <button onClick={(e) => { e.stopPropagation(); toggleStageExpanded(s.k); }}
                                    title={expandedStages[s.k] ? "Réduire (≈ 2 cartes)" : ("Déployer toutes les opportunités (" + opps.length + ")")}
                                    style={{ width: 22, height: 22, padding: 0, border: "1px solid " + (expandedStages[s.k] ? s.color : "#e2e8f0"),
                                             background: expandedStages[s.k] ? s.color + "15" : "#fff",
                                             color: expandedStages[s.k] ? s.color : "#64748b", borderRadius: 5, cursor: "pointer",
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: expandedStages[s.k] ? "none" : 296, overflowY: "auto", paddingRight: 4, marginRight: -4 }}
                      ...
                      {opps.map((o, j) => {
                        const openOpp = () => {
                          const cid = urlId || display.id || "";
                          const oppKey = o.ref || o.id;
                          if (!oppKey) { if (window.HubToast) window.HubToast.warn("Référence opportunité manquante"); return; }
                        ...
                        const proba = o.proba || ({ qualif: 20, discovery: 35, propo: 55, nego: 75, won: 100 }[o.stage] || 20);
                        const isWon = o.stage === "won";
                        const logoPalette = ["#f59e0b", "#0ea5e9", "#a855f7", "#dc2626", "#10b981", "#6366f1"];
                        const logoBg = logoPalette[j % logoPalette.length];
                        const logo = ((o.client_name || display.name || "??").slice(0, 2)).toUpperCase();
                        const tagLabel = (Array.isArray(o.modules) && o.modules[0]) || (o.produit && o.produit.includes("Cyber") ? "Cyber" : o.produit && o.produit.includes("Hub") ? "Hub" : "Suite");
                        const tagBg = tagLabel === "Cyber" ? "#fdecec" : tagLabel === "Hub" ? "#eef2ff" : "#f5efff";
                        ...
                        return (
                          <div key={o.ref || j} onClick={openOpp}
                            ...
                            <div style={{ display: "flex", alignItems: "flex-start", gap: 9 }}>
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontSize: 12.5, fontWeight: 600, color: "#0f172a", lineHeight: 1.3, wordBreak: "break-word" }}>{o.name || "—"}</div>
                                <div style={{ fontSize: 11, color: "#64748b", marginTop: 1 }}>{o.client_name || display.name || "—"}</div>
                                <div style={{ display: "flex", gap: 4, marginTop: 4 }}>
                                  <span style={{ display: "inline-block", padding: "1px 6px", borderRadius: 4, fontSize: 10, fontWeight: 600, background: tagBg, color: tagColor }}>{tagLabel}</span>
                                  {isWon && <span style={{ display: "inline-block", padding: "1px 6px", borderRadius: 4, fontSize: 10, fontWeight: 600, background: "#e8f8f1", color: "#0e7a55" }}>✓ Signé</span>}
                                </div>
                              </div>
                            </div>
                            <div style={{ fontSize: 18, fontWeight: 600, color: "#0f172a", letterSpacing: -0.4 }}>{o.amount}</div>
                            <div>
                            ...
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 4, paddingTop: 8, borderTop: "1px solid #f1f5f9" }}>
                              <Avatar name={o.owner} size={20} color={o.ownerColor || stageColor} />
                              <span style={{ fontSize: 11, color: "#64748b", display: "inline-flex", alignItems: "center", gap: 3 }}>
                                <span style={{ color: "#94a3b8" }}>◷</span>0j
                              </span>
                            </div>
                          </div>
                        );
                      ...
                      {/* Bouton + Ajouter une opportunité pré-positionné sur l'étape */}
                      <a
                  ...
                  return (
                    <div style={{ padding: 4 }}>
                      <Section title="Identité légale" icon="🏛" color="#7c3aed">
                        ...
                        {c.pappers && Array.isArray(c.pappers.dirigeants) && c.pappers.dirigeants.length > 0 && (
                        ...
                        {c.pappers && Array.isArray(c.pappers.procedures) && c.pappers.procedures.length > 0 && (
