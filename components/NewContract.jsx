...
const NewContract = () => {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      const hit = local.find((p) => p.id === clientId);
      if (hit) { setClientObj(hit); return; }
    } catch (e) {}
    if (window.HubData && window.HubData.fetchClientById) {
      window.HubData.fetchClientById(clientId).then(({ data }) => { if (data) setClientObj(data); });
    }
  }, [clientId]);
