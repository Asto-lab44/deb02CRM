...
const HotlinePopup = ({ call, onClose, onCreateTicket }) => {
  ...
  const [phase, setPhase] = React.useState("ringing"); // ringing | answered | newTicket
  ...
  const answer = () => {
    setPhase(call.openTickets && call.openTickets.length > 0 ? "answered" : "newTicket");
  };
      ...
      <div style={{ ...H.modal, ...(phase === "ringing" ? H.modalRinging : {}) }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={H.head}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 11.5, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 0.6 }}>
                {phase === "ringing" ? "Appel entrant · 3CX" : phase === "newTicket" ? "Nouveau ticket" : "En ligne · " + fmtElapsed()}
              </div>
        ...
        {phase === "answered" && (
          <div style={H.body}>
            ...
            <div style={H.ringingFoot}>
              <button onClick={onClose} style={{ ...H.btn, ...H.btnDecline, background: "#fee2e2", color: "#dc2626" }}>⏻ Raccrocher</button>
              <button onClick={() => setPhase("newTicket")} style={{ ...H.btn, ...H.btnAnswer, background: "#3730a3" }}>+ Créer un ticket</button>
            </div>
          </div>
        )}
...
const HOTLINE_DEMO_CALLERS = [
  ...
  },
  {
    ...
    openTickets: [], // → ouvre le formulaire nouveau ticket
  },
  {
    id: "c3", phone: "+33 2 51 47 88 02", line: "Hotline support",
    name: "Camille Renard", company: "Allianz France", email: "c.renard@allianz.fr",
    tier: "premium", contract: "Premium 24/7", lastContact: "hier (Léa M.)",
    openTickets: [
      { id: "INC-2837", title: "Outlook : pièces jointes >25 Mo bloquées", prio: "normale", sla: "ok", slaLeft: "5 h 10", opened: "il y a 2 j", assignee: "Karim Ben Salah" },
    ],
  },
  {
