    ...
    <div style={ST.frame}>
      <header style={ST.topbar}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 12.5, color: "#64748b" }}>
          <a href={isTest ? "/crm?test=1" : "/crm"} style={{ color: "#64748b", textDecoration: "none" }}>{isTest ? "CRM (test)" : "CRM"}</a>
      ...
      {loading ? <div style={ST.empty}>Chargement…</div> : (
        <div style={ST.body}>
          {groupKeys.length === 0 && unknown.length === 0 && <div style={ST.empty}>Aucun client hors 44 (ou aucun code postal renseigné — lancez l'enrichissement Pappers depuis le CRM test).</div>}
