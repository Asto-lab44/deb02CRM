...
var ERPHome = () => {
  ...
  var defaultGroup = {
    ...
    color: "#dc2626",
    access: ["crm", "crm_test", "intel", "marketing", "tech", "projects", "commercial", "contracts", "inventory", "suppliers", "accounting", "billing", "treasury", "hr", "time", "reports", "settings"]
  };
  ...
  var [commercialStats, setCommercialStats] = React.useState({
    devis: 0,
    factures: 0,
    encours: 0
  });
  React.useEffect(() => {
    ...
    (async () => {
      try {
        var [devis, factures] = await Promise.all([window.api.commercialDocs.list({
          type: "devis"
        }), window.api.commercialDocs.list({
          type: "facture"
        })]);
        var encours = (factures || []).filter(f => f.status !== "paye" && f.status !== "annule").reduce((s, f) => s + (Number(f.total_ttc) || 0), 0);
        setCommercialStats({
          devis: (devis || []).length,
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        var s = await window.api.userActivity.dashboardStats();
        setActivityStats({
          online_now: s.online_now || 0,
          sessions_today: s.sessions_today || 0,
          errors_today: s.errors_today || 0
        });
      } catch (e) {}
    })();
  }, []);
  ...
  // FINANCE
  {
    ...
    stats: [{
      k: "Devis",
      v: String(commercialStats.devis)
    }, {
      k: "Factures",
      v: String(commercialStats.factures)
    }, {
      k: "Encours",
      v: fmtKE(commercialStats.encours)
    }],
    badge: {
      label: "Sage-like",
      tone: "info"
    ...
    icon: /*#__PURE__*/React.createElement("svg", {
      ...
      d: "M8 2h8l4 4v14a2 2 0 01-2 2H6a2 2 0 01-2-2V4a2 2 0 012-2z"
    }), /*#__PURE__*/React.createElement("path", {
  ...
  // RH
  {
    ...
    stats: [{
      k: "En ligne",
      v: String(activityStats.online_now)
    }, {
      k: "Sessions/j",
      v: String(activityStats.sessions_today)
    }, {
      k: "Erreurs",
      v: String(activityStats.errors_today)
    }],
    badge: {
      label: "À valider",
      tone: "warn"
